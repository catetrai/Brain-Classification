...
model.load_state_dict(torch.load(os.path.join("./model_weight", "best_metric_model.pth"), map_location=torch.device('cpu') if not torch.cuda.is_available() else None))
model.eval()
